...
int main() {
    ...
    printf("\nProducer ready to create items for consumer\n");

    // Loop to complete work
    for(int i = 0; i < iterations; i++) {
        // Block producer until there is room in the table to produce at least one item
        if(sem_wait(CanProduce) != 0) {
            exitError("Failure in sem_wait(CanProduce)\n", table, tbl, CanConsume, CanProduce, Mutex);
        }
        ...
        printf("Item Produced, %d item(s) left remaining in the table.\n", *table);
        
        // Signal that an additional item can be consumed
        if(sem_post(CanConsume) != 0) {
